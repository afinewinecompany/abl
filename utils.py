...
def save_player_points(data: dict, file_path: str = 'data/player_points.csv') -> bool:
    """
    Save player points data to a CSV file
    
    Args:
        data: Dictionary with player names as keys and points as values
        file_path: Path to save the CSV file
    ...
    try:
        ...
        if data:
            ...
            df = pd.DataFrame(list(data.items()), columns=['player_name', 'actual_points'])
            df.to_csv(file_path, index=False)
            return True
        
        return False
    except Exception as e:
        st.error(f"Error saving player points data: {str(e)}")
        return False

def load_player_points(file_path: str = 'data/player_points.csv') -> dict:
    """
    Load player points data from a CSV file
    
    Args:
        file_path: Path to the CSV file
    
    Returns:
        dict: Dictionary with player names as keys and points as values
    """
    try:
        if os.path.exists(file_path):
            df = pd.read_csv(file_path)
            ...
            player_points = dict(zip(df['player_name'], df['actual_points']))
            return player_points
        
        return {}
    except Exception as e:
        st.error(f"Error loading player points data: {str(e)}")
        return {}
